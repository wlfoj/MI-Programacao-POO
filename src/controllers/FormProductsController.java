...
public class FormProductsController implements Initializable {
    ...
    /**Metodo para salvar as informaÁıes digitadas do produto, seja para criar ou para editar
     * 
	...
	private void eventSave(ActionEvent event) throws IOException{
		...
		if (Main.getIdSelected() == -1) {
			try {
				...
				AlertsController.alertErrorDate(e.getMessage(), "Valor do produto negativo","Usu√°rio por favor preencher com valores acima de 0" );
			} catch (InsufficientQuantityProducts e) {
				AlertsController.alertErrorDate(e.getMessage(), "Quantidade de produtos insuficiente","Usu√°rio por favor preencher com quantidade de produtos suficiente" );
			} catch (DateInvalid e) {
				AlertsController.alertErrorDate(e.getMessage(),"Data de validade anterior a atual", "Usu√°rio por favor preencher com data posterior a atual" );
			}
    	...
    	// Se passar pelas etapas sem receber uma exceÁ„o, volta para tela anterior
    	if(aux==false) {
    		backToProduct();
    	}
	}
	 ...
	 * 
	 * @throws NegativePriceEntity - Excecao para valores de preÁo negativos
